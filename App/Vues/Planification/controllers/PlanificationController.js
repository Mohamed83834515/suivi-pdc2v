DECAISSEMENT.controller('PlanificationController', function ($rootScope, $scope, $http, $cookies, $filter, $location, $routeParams, $httpParamSerializer) {
	...
	$scope.getActiviteChange = function (version) {
		...
		$http({
			method: 'GET',
			//url: 'https://sise-pdc2v.org/api/priotaire1.php?responsable=' + $scope.nom,
	...
	$scope.getListeActiviteChange = function (version) {
		...
		$http({
			method: 'GET',
			url: $scope.link + '/api/priotaire.php?responsable=' + $scope.nom + '&id_personnel=' + $scope.cokk + '&Fonction=' + $scope.Fonction + '&Partenaire=' + $scope.Partenaire+ '&annee='+version,
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}).success(function (data) {
			$scope.ListeActivite = data.activites;
		...
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					...
					$http.delete($scope.link + '/api/supprimer_planification.php?id_suivi=' + id_activite_ptba_1)
