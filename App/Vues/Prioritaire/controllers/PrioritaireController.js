DECAISSEMENT.controller('PrioritaireController', function ($rootScope, $timeout, $scope, $http, $cookies, $filter, $location, $routeParams, $httpParamSerializer) {
    ...
    $('#selectElement1').on('change', function() {
        $scope.$apply(function() {
          $scope.Motif.Responsable = $('#selectElement1').val();
        });
    });
	$('#selectElement3').select2({
        dropdownParent: $('#exampleModal3')
    });
    $('#selectElement3').on('change', function() {
        $scope.$apply(function() {
          $scope.Motif.Responsable = $('#selectElement3').val();
        });
    });
	...
	$scope.getSuiviActivite = function (id) {
		$scope.suividata = [];
		$scope.Motif.IdTache = id;
		$scope.id_activite_ptba_1 = id;
		for (var i = 0; i <= $scope.ListePlanification.length - 1; i++) {
			if ($scope.ListePlanification[i].contrat == id) {
				$scope.suividata.push($scope.ListePlanification[i]);
			}
	...
	$scope.getListeActivite = function () {
		...
		$http({
			...
			// Une fois que vous avez récupéré les activités, 
		});
	}
	...
	$scope.ValiderPlanification = function (id_activite_ptba_1) {
		swal({
			title: "Etes vous sur?",
			...
			.then((willEdit) => {
				if (willEdit) {
					...
					$scope.AddPlanification(id_activite_ptba_1);
	...
	$scope.AddPlanification = function (id_activite_ptba_1) {
		...
		fd.append('activite', $scope.Motif.IdTache);
		fd.append('tache', $scope.Motif.tache);
		$http({
			method: 'post',
			url: $scope.link + '/api/insert_planification.php',
			data: fd,
	...
	$scope.getPlanificationById = function (IdMotif) {
		...
		for (var i = 0; i <= $scope.ListePlanification.length - 1; i++) {
			if ($scope.ListePlanification[i].id_suivi == IdMotif) {
				...
				$timeout(function () {
					$('#selectElement3').val($scope.Motif.Responsable).trigger('change');
				});
				break;
			}
		}
	...
	$scope.UpdatePlanification = function (id_activite_ptba_1) {
		var dateReelleFormatted = formatDate($scope.Motif.Delai);
		var nom = $cookies.get("NomUtilisateur");
		var fd = new FormData();
		fd.append('ResponsableUCP', nom);
		fd.append('id_suivi', $scope.Motif.CodeTache);
		fd.append('Responsable', $scope.Motif.Responsable);
		fd.append('Delai', dateReelleFormatted);
		fd.append('activite', $scope.Motif.IdTache);
		fd.append('tache', $scope.Motif.tache);
		$http({
			method: 'post',
			url: $scope.link + '/api/update_planification.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
	...
	$scope.ValideDeleteById = function (id,id_activite_ptba_1) {
		swal({
			title: "Etes vous sur?",
			...
			.then((willDelete) => {
				if (willDelete) {
					...
					$http.delete($scope.link + '/api/supprimer_planification.php?id_suivi=' + id)
	...
	$scope.confirmDelete = function (Motif, id_activite_ptba_1) {
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					// Appel à l'API PHP pour supprimer l'élément
					$http.delete($scope.link + '/api/supprimer_suivi_tache.php?id_suivi=' + Motif.id_suivi)
						.then(function (response) {
							$scope.InitialiseActivite();
	...
	$scope.UpdateHistorique = function () {
		...
		var fd = new FormData();
		var files = document.getElementById('file121').files[0];
		...
		$http({
			method: 'post',
			url: $scope.link + '/api/update_suivi_tache.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
	...
	$scope.getTacheById = function (IdMotif) {
		for (var i = 0; i <= $scope.ListeSousActivite.length - 1; i++) {
			if ($scope.ListeSousActivite[i].id_suivi == IdMotif) {
				$scope.Motif.IdTache = $scope.ListeSousActivite[i].id_tache ;
				$scope.Motif.CodeTache = $scope.ListeSousActivite[i].source_verification;
				$scope.Motif.Livrable = $scope.ListeSousActivite[i].source_verification;
				$scope.Motif.Nlot = $scope.ListeSousActivite[i].id_suivi;
				$scope.Motif.DateReelle = new Date($scope.ListeSousActivite[i].date_suivi);
				$scope.Motif.Observation = $scope.ListeSousActivite[i].etat_avancement;
				$scope.Motif.StatutTache = $scope.ListeSousActivite[i].statut;
				$scope.Motif.Difficultes = $scope.ListeSousActivite[i].difficultes;
				break;
			}
		}
	}
	...
	$scope.UpdateMotif = function () {
		...
		$http({
			method: 'post',
			url: $scope.link + '/api/insert_suivi_tache.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
