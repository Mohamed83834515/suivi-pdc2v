DECAISSEMENT.controller('RecommandationController', function ($rootScope, $scope, $http, $cookies, $filter, $location, $routeParams, $httpParamSerializer) {
	...
	$scope.getListeRubrique = function () {
		...
		$http({
			method: 'GET',
			url: $scope.link + '/api/Rubrique.php',
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}).success(function (data) {
		...
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					...
					$http.delete($scope.link + '/api/supprimer_suivi_plan_mission.php?id_suivi=' + mission)
		...
		swal({
			...
			.then((willEdit) => {
				if (willEdit) {
					...
					$http({
						method: 'post',
						url: $scope.link + '/api/reset_plan_mission.php',
						data: fd,
	...
	$scope.ValiderSuiviMission = function (suivi) {
		...
		$http({
			method: 'post',
			url: $scope.link + '/api/valider_plan_mission.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
	...
	$scope.telechargerFichier = function (nomFichier) {
		...
		a.href =$scope.link + '/api/fichierSuiviMission/' + nomFichier;
	...
	$scope.UpdateSuiviMission = function () {
		...
		$http({
			method: 'post',
			url: $scope.link + '/api/update_plan_mission.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
	...
	$scope.AddSuiviMission = function () {
		...
		$http({
			method: 'post',
			url: $scope.link + '/api/inser_suivi_plan_mission.php',
			data: fd,
			headers: { 'Content-Type': undefined },
		}).then(function successCallback(response) {
			$scope.response = response.data
	...
	$scope.getListeMissionPlan = function () {
		...
		$http({
			method: 'GET',
			url: $scope.link + '/api/mission_plan.php',
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}).success(function (data) {
	...
	$scope.getListeRecommandation = function (mission_id) {
		...
		$http({
			method: 'GET',
			url: $scope.link + '/api/recommandation.php?id_mission=' + mission_id,
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}).success(function (data) {
	...
	$scope.getListeMissionSupervision = function () {
		...
		$http({
			method: 'GET',
			url: $scope.link + '/api/mission_supervision.php?annee=' + 1,
			headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		}).success(function (data) {
	...
	$scope.ValideDeleteById = function (id_activite_ptba_1) {
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					// Appel à l'API PHP pour supprimer l'élément
					$http.delete($scope.link + '/api/supprimer_Recommandation.php?id_suivi=' + id_activite_ptba_1)
						.then(function (response) {
							$scope.InitialiseRecommandation();
