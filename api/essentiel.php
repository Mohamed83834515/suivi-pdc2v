...
function getRelativeTime($date)
{
  ...
  function do_link($id="", $href="",$title="",$val="",$type="",$path="./",$class="",$onclick="",$pop=0,$style="",$page="",$onclickOnly=0)
  {
    ...
    switch($type) {
      ...
      case 'add':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-plus" title="Ajouter" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'view':  
      //if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-eye" title="Afficher" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'edit':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-edit" title="Modifier" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'del':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-trash-alt" title="Supprimer" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'access':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-balance-scale" title="Droits" style="font-size: 18px;color: red;"></i>';
      break;
      case 'valid':
      if(check_user_auth('page_valid',$page)) return;
      $img = '<i class="icon-check-circle" title="Valider" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'verif':
      if(check_user_auth('page_verif',$page)) return;
      $img = '<i class="icon-check-double" title="V&eacute;rifier" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'import':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-cloud-upload-alt" title="Importer" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'export':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-cloud-download-alt" title="Importer" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'reload':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-sync-alt" title="Actualiser" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'send_message':
      if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-paper-plane" title="Envoyer" style="font-size: 18px;color: #929292;"></i>';
      break;
      case 'download':
      //if(check_user_auth('page_edit',$page)) return;
      $img = '<i class="icon-cloud-download-alt" title="T&eacute;l&eacute;charger" style="font-size: 18px;color: #929292;"></i>';    
      break;
    }
