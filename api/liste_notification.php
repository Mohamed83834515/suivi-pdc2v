...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['responsable'])) {
        ...
        $query = "SELECT * FROM notifications WHERE statut = 0 AND Responsable = '$userId' ORDER BY id DESC";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo json_encode(['message' => 'Notification récupérées avec succès', 'status' => 'success', 'notif' => $sous_activites]);
        } else {
            // Aucune tâche trouvée
            http_response_code(404); // Non trouvé
            echo json_encode(['message' => 'Aucune Notification trouvée pour cet utilisateur', 'status' => 'error']);
        }
    } else {
