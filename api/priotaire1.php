...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
   ...
   // echo $fonction;
        ...
        if($partenaire == 1)
            ...
            {
                $annee= $_GET['annee'];
                $query = "SELECT * FROM ptba WHERE annee='$annee' AND volet_ref = '1'  ORDER BY code_activite_ptba ASC";  
            }
            else
            {
                $query = " SELECT * FROM ptba WHERE annee = (SELECT MAX(annee) FROM ptba) AND volet_ref = '1'  ORDER BY code_activite_ptba ASC";
            }
        else {
            if(isset($_GET['annee']))
            {
                $annee= $_GET['annee'];
                $query = "SELECT * FROM ptba WHERE annee='$annee' AND ( executant = '$userId' OR responsable = '$fonction' ) AND volet_ref = '1'  ORDER BY code_activite_ptba ASC";  
            }
            else
            {
                $query = " SELECT * FROM ptba WHERE annee = (SELECT MAX(annee) FROM ptba WHERE executant = '$userId' OR responsable = '$fonction') AND volet_ref = '1' AND ( executant = '$userId' OR responsable = '$fonction' )  ORDER BY code_activite_ptba ASC";
            }
        }    
