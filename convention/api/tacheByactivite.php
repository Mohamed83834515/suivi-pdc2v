...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['responsable'])) {
        ...
        $userId = $_GET['responsable'];  // Vérifier si le paramètre d'authentification (ID utilisateur) est présent
        $query = "SELECT * FROM suivi_avenant WHERE id_personnel = '$userId'  AND TIMESTAMPDIFF(day,now(),date_avenant)<=7 AND TIMESTAMPDIFF(day,now(),date_avenant)>=0 ORDER BY date_enregistrement DESC;";
