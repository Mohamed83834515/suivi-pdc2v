...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_personnel']) ) {
        $id_v = isset($_POST['id_personnel']) ? $_POST['id_personnel'] : null;
        $convention_active = isset($_POST['convention']) ? $_POST['convention'] : null;
        ...
        $updateQuery = "UPDATE personnel SET
            convention_active = '$convention_active'
            WHERE N = '$id_v'";

        if ($conn->query($updateQuery) === TRUE) {
            // Mise à jour réussie
            http_response_code(200); // OK
            echo json_encode(['message' => 'Personnel modifié avec succès', 'status' => 'success']);
        } else {
            // Échec de la mise à jour
            http_response_code(500); // Erreur interne du serveur
            echo json_encode(['message' => 'Échec de la modification du Personnel', 'status' => 'error', 'error' => $conn->error]);
        }
    } else {
