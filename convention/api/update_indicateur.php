...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_indicateur']) && isset($_POST['valeur_cibles'])) {
        ...
        $insertQuery1 = "DELETE FROM `suivi_indicateur_convention` WHERE indicateur = $id_indicateur2 ";
        if ($conn->query($insertQuery1) === TRUE) {
            for ($i = 0; $i < count($valeur_cibles); $i++) {
                // $insertQuery = "UPDATE  suivi_indicateur_convention  SET  date_suivi = '$date_reelle', valeur_suivi = $valeur_cibles[$i] WHERE id_suivi = $id_indicateur[$i] ";
              
                $insertQuery = "INSERT INTO suivi_indicateur_convention (indicateur, date_suivi, valeur_suivi, mois, commune) VALUES ";
