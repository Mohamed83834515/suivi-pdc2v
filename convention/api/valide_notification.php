...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_notif']) ) {
        $id_notif = isset($_POST['id_notif']) ? $_POST['id_notif'] : null;
        ...
        if ($conn->query($updateQuery) === TRUE) {
            ...
            echo json_encode(['message' => 'notifications modifié avec succès', 'status' => 'success']);
        } else {
            // Échec de la mise à jour
            http_response_code(500); // Erreur interne du serveur
            echo json_encode(['message' => 'Échec de la modification du notifications', 'status' => 'error', 'error' => $conn->error]);
        }
    } else {
