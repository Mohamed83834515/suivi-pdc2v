...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if (isset($_GET['id_suivi'])) {
        ...
        $deleteQuery = "DELETE FROM suivi_mission WHERE id_suivi = '$taskId'";

        if ($conn->query($deleteQuery) === TRUE) {
            // Suppression réussie
            http_response_code(200); // OK
            echo json_encode(['message' => 'mission supprimé avec succès', 'status' => 'success']);
        } else {
            // Échec de la suppression
            http_response_code(500); // Erreur interne du serveur
            echo json_encode(['message' => 'Échec de la suppression du mission', 'status' => 'error', 'error' => $conn->error]);
        }
    } else {
