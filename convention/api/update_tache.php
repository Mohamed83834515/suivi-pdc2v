...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_tache']) && isset($_POST['code_tache']) && isset($_POST['lot'])) {
        ...
        $insertQuery = "INSERT INTO suivi_avancement_contrat_convention
            (id_activite_ptba, date_suivi, code_suivi, statut_activite, difficultes_rencontrees, pistes_solutions, observation, documents, id_personnel)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param("sssssssss", $taskId, $newDateFin, $lot, $statut_tache, $difficultes, $pistes_solutions, $newObservation, $filename, $id_personnel);
        
        // Exécution de la requête
        if ($stmt->execute()) {
            // Insertion réussie
            http_response_code(200); // OK
            echo json_encode(['message' => 'Tâche ajoutée avec succès', 'status' => 'success']);
        } else {
            // Échec de l'insertion
            http_response_code(500); // Erreur interne du serveur
            echo json_encode(['message' => 'Échec de l\'ajout de la tâche', 'status' => 'error', 'error' => $stmt->error]);
        }
